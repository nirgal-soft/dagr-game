...
pub fn draw(
    ...
    if matches.is_empty() {
        ...
        for (index, choice) in matches.iter().take(6).enumerate() {
            let marker = if index == picker.selected_index() {
                ">"
            } else {
                " "
            };
            ...
            let content = format!(
                "{marker} {:<24} HD {:>2}  AC {:>2}",
                choice.name, hd, choice.armor_class
            );
            text(
                stdout,
                x + 2,
                y + 4 + index as u16,
                &truncate(&content, inner.saturating_sub(2)),
    ...
    text(
        ...
        x + 2,
        y + height - 2,
        "Type to filter · ↑/↓ select · Enter spawn · Esc cancel",
        Color::DarkGrey,
    )?;
