...
impl Popup{
  ...
  pub fn draw(&self, stdout: &mut Stdout) -> Result<()>{
    ...
    queue!(stdout, style::PrintStyledContent(top.with(Color::White).on(Color::DarkBlue)))?;

    queue!(stdout, cursor::MoveTo(x, y + 1))?;
    let side = format!("│{}│", " ".repeat((box_width - 2) as usize));
    queue!(stdout, style::PrintStyledContent(side.with(Color::White).on(Color::DarkBlue)))?;
    ...
    queue!(stdout, style::PrintStyledContent(msg_line.with(Color::Yellow).on(Color::DarkBlue)))?;
    ...
    queue!(stdout, style::PrintStyledContent(hint_line.with(Color::Grey).on(Color::DarkBlue)))?;

    queue!(stdout, cursor::MoveTo(x, y + 4))?;
    let bottom = format!("└{}┘", "─".repeat((box_width - 2) as usize));
    queue!(stdout, style::PrintStyledContent(bottom.with(Color::White).on(Color::DarkBlue)))?;
