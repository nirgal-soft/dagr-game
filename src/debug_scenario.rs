...
async fn create_npc(engine: &Engine, name: &str) -> Result<dagr_lib::characters::CharacterId> {
    Ok(engine
        .characters()
...
mod tests {
    ...
    async fn demo_preserves_the_authored_fixture() {
        ...
        assert_eq!(
            front.stakes[0].description,
            "Who will break the Compact's hold on the valley?"
        );
