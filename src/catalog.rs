...
impl CatalogStore {
    pub fn discover() -> Result<Self> {
        let base = BaseDirs::new().context("platform application-data directory is unavailable")?;
    ...
    pub fn load(&self) -> Result<Catalog> {
        ...
        let reader = BufReader::new(
            File::open(&path)
                .with_context(|| format!("open managed catalog {}", path.display()))?,
        );
    ...
    pub fn save(&self, catalog: &Catalog) -> Result<()> {
            ...
            .join(format!(".{CATALOG_FILE}.{}.tmp", std::process::id()));
        {
            ...
            serde_json::to_writer_pretty(&mut writer, catalog)
                .context("serialize managed catalog")?;
            writer.write_all(b"\n")?;
            writer.flush()?;
    ...
    for (id, world) in &catalog.worlds {
        ...
        if world.setup_state == SetupState::Ready {
            let binding = catalog
                ...
                .get(id)
                .with_context(|| {
                    format!("ready managed world '{id}' has no local-player binding")
                })?;
