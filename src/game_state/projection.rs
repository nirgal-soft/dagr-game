...
impl GameState {
  pub(super) async fn restore_view(&mut self, actor: CharacterId) -> Result<()> {
    ...
    let area = match destination.facts().kind {
      ...
      LocationKind::DungeonRoom => {
        ...
        let LocationView::Dungeon(dungeon) = self.engine.world().location(parent).await? else {
  ...
  pub(super) async fn apply_travel(&mut self, outcome: TravelOutcome) -> Result<()> {
    match &outcome.area {
      ...
      Some(AreaView::Wilderness(wilderness)) => {
        if !self
          .view_manager
          .cached_area(wilderness.location_id, None)
        {
  ...
  async fn build_wilderness_area(&self, wilderness: &WildernessView) -> Result<crate::areas::Area> {
    ...
    if let Some(parent) = parent {
      for child in self.engine.world().children(parent).await? {
        if child.kind == LocationKind::Dungeon {
          if let LocationView::Dungeon(dungeon) = self.engine.world().location(child.id).await? {
            return Ok(dungeon);
          }
        }
      }
