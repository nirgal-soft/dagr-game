...
use dagr_lib::agency::{
  AnthropicTransportConfig, ModelDecision, ModelOptions, ModelProfile, ModelProvider, ModelRequest,
  ModelResponse, ProviderKind, ToolCall,
};
...
impl ModelProvider for AnthropicProvider {
  ...
  async fn decide(
    ...
    if !status.is_success() {
      return Err(
        anyhow::anyhow!("Anthropic model request failed with status {status}: {body}").into(),
      );
    }
...
fn response_from_body(body: &str) -> Result<ModelResponse> {
  ...
  for block in response.content {
    match block {
      AnthropicContent::ToolUse { id, name, input } => calls.push(ToolCall { id, name, input }),
