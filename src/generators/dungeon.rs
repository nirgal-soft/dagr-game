...
use dagr_lib::components::world::{
  dungeon::Dungeon,
  dungeon_connection::{ConnectionType, DungeonConnection},
...
impl DungeonGenerator {
  ...
  pub fn generate(
    ...
    let mut room_spatials: HashMap<LocationId, SpatialData> = HashMap::new();
    ...
    // Dungeon ground represents carved floor. Start with solid rock and carve
    // rooms and passages out of it so space outside the layout is impassable.
    for y in 0..height {
      for x in 0..width {
        area.set_wall(x, y);
      }
    ...
    for entity in passage_entities {
      ...
      if passage_data.get_dungeon_level() == target_level {
        ...
        if let (Some(from_room), Some(to_room)) =
          (room_spatials.get(&from_id), room_spatials.get(&to_id))
        {
    ...
    let connection_entities =
      entity_manager.find_all_children::<DungeonConnection>(dungeon_location_id);
