...
impl ToolForm {
  ...
  fn build(&self) -> std::result::Result<(String, Value), String> {
    ...
    for field in &self.fields {
      ...
      let value = parse_form_value(field).map_err(|error| format!("{}: {error}", field.name))?;
...
fn parse_form_value(field: &FormField) -> std::result::Result<Value, String> {
  match field.kind.as_str() {
    ...
    _ => serde_json::from_str(&field.value).map_err(|error| format!("enter valid JSON ({error})")),
  }
}
...
fn draw_form(frame: &mut Frame<'_>, area: Rect, form: &ToolForm) {
  ...
  let items: Vec<ListItem> = form
    ...
    .collect();
  let mut state = ListState::default().with_selected((!items.is_empty()).then_some(form.selected));
