...
impl Navigator {
    ...
    pub fn navigate_to_nearest(&mut self, start: Pos, goals: Vec<Pos>, area: &Area) -> Result<()> {
        if goals.is_empty() {
            return Err(anyhow!(NavigationError::NoGoalsVisible));
        }

        let result = find_path_to_nearest(start, goals, |x, y| area.is_seen_and_walkable(x, y));
    ...
    pub fn find_exploration_target(&mut self, start: Pos, area: &Area) -> Result<()> {
