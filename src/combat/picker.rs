use dagr_lib::definitions::monster::MonsterDefinition;
...
impl From<MonsterDefinition> for MonsterChoice{
  fn from(definition: MonsterDefinition) -> Self{
    Self{
...
impl MonsterPicker{
  pub fn new(definitions: Vec<MonsterDefinition>) -> Self{
    Self{
      query: String::new(),
  ...
  pub fn matches(&self) -> Vec<&MonsterChoice>{
    let mut matches = self.choices.iter()
      .filter_map(|choice| fuzzy_score(&self.query, &format!("{} {}", choice.name, choice.key))
...
mod tests{
  ...
  fn choice(name: &str, key: &str) -> MonsterChoice{
    MonsterChoice{name:name.into(),key:key.into(),hit_dice:vec![1,6,0],armor_class:3}
  }
  ...
  fn selection_wraps_filtered_choices(){
    let mut picker=MonsterPicker{query:String::new(),choices:vec![choice("Boar","core:boar"),choice("Goblin","core:goblin")],selected:0};
