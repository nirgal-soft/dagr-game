...
use crate::ui::{
    command_line, map::Map, monster_picker, panel::Panel, popup::Popup, stat_bar::StatBar,
};
use anyhow::Result;
...
impl Renderer {
    ...
    fn render_hexmap(
        ...
        map.draw(stdout, |x, y| {
            ...
            let mut tile = if pos == (game_state.player_x, game_state.player_y) {
                ...
                game_state
                    .world_tile(world_x, world_y)
                    .or_else(|| Some(Tile::new('░', Color::DarkGrey)))
            };
            if game_state
                .debug
                .editor()
                .is_some_and(|editor| editor.cursor == pos)
            {
    ...
    fn render_location(
        ...
        map.draw(stdout, |x, y| {
            let pos = (
                x as i32 + game_state.camera.x,
                y as i32 + game_state.camera.y,
            );
            self.location_tile_at(
                game_state,
                &enemies,
                pos,
                game_state.look_cursor() == Some(pos),
            )
        })?;
        Ok(())
    }
    ...
    pub fn render_look_delta(
        ...
        if let Some((x, y)) = game_state.camera.world_to_screen(previous.0, previous.1) {
            map.draw_cell(
                ...
                x,
                y,
                self.location_tile_at(game_state, &enemies, previous, false),
            )?;
        }
        if let Some(current) = game_state.look_cursor() {
            if let Some((x, y)) = game_state.camera.world_to_screen(current.0, current.1) {
                map.draw_cell(
                    ...
                    x,
                    y,
                    self.location_tile_at(game_state, &enemies, current, true),
                )?;
            }
        }
        self.render_ui(stdout, game_state)?;
    ...
    fn location_tile_at(
        ...
        if highlighted {
            tile = Some(
                tile.unwrap_or_else(|| Tile::new(' ', Color::White))
                    .inverted(),
            );
        }
        tile
    ...
    fn render_ui(&self, stdout: &mut std::io::Stdout, game_state: &GameState) -> Result<()> {
        ...
        if combat_width >= 20 {
            ...
            if let Some(editor) = game_state.debug.editor() {
                ...
                if let Some(message) = game_state.debug.message() {
